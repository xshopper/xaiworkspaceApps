...
export class LitellmClient {
  ...
  /**
   * Update a virtual key (e.g., change budget, add/remove models).
   * @param {string} keyId
   * @param {object} updates
   */
  async updateVirtualKey(keyId, updates) {
    const resp = await fetch(`${this.baseUrl}/key/update`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
        Authorization: `Bearer ${this.masterKey}`,
    ...
    if (!resp.ok) {
      const text = await resp.text().catch(() => '');
      throw new Error(`LiteLLM /key/update failed (${resp.status}): ${text}`);
    }
  ...
  /**
   * List all virtual keys with optional metadata filter.
   * @param {{ metadata?: object }} opts
   */
  async listVirtualKeys(opts = {}) {
    ...
    const resp = await fetch(`${this.baseUrl}/key/info?${params}`, {
      headers: { Authorization: `Bearer ${this.masterKey}` },
    });

    if (!resp.ok) {
      const text = await resp.text().catch(() => '');
      throw new Error(`LiteLLM /key/info failed (${resp.status}): ${text}`);
    }
   ...
   * @returns {Promise<{ data: Array<{ id: string, owned_by: string }> }>}
   */
  async getModels() {
    ...
    if (!resp.ok) {
      const text = await resp.text().catch(() => '');
      throw new Error(`LiteLLM /v1/models failed (${resp.status}): ${text}`);
    }
