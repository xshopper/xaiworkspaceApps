/**
 * Claude Code panel — TypeScript source
 *
 * Compile to panel.js for deployment: esbuild --format=iife
 * The platform loads ui/panel.js inside the sandbox iframe.
 *
 * Communication:
 *   Panel → server.js  via xai.http('http://localhost:3457/query', ...)
 *   server.js → Claude Code via @anthropic-ai/claude-agent-sdk
 */
...
interface OutputEntry {
  type: 'system' | 'cmd' | 'response' | 'error' | 'info';
  text: string;
}
...
function renderMd(text: string): string {
  ...
  html = html.replace(/\*\*([^*\n]+)\*\*/g, (_m: string, t: string) => `<strong>${esc(t)}</strong>`);
  html = html.replace(/\n/g, '<br>');
  return html;
}
...
async function checkHealth(): Promise<boolean> {
  try {
    const resp = await xai.http<HealthData>(`${SERVER}/health`);
...
async function loadSessions(): Promise<void> {
  try {
    const resp = await xai.http<{ ok: boolean; sessions: SessionInfo[] }>(`${SERVER}/sessions`);
...
function buildStatusCard(): string {
  if (!state.health) {
    return `
...
function buildCommandBar(): string {
...
function render(): void {
  ...
  (window as any).__stop = stopQuery;
  (window as any).__cmd = runCommand;
  (window as any).__toggleSessions = toggleSessions;
  (window as any).__resumeSession = resumeSession;
...
async function run(): Promise<void> {
  ...
  try {
    const resp = await xai.http<{ ok: boolean; output?: string; error?: string; sessionId?: string }>(
      `${SERVER}/query`,
      {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ prompt: text, sessionId: state.sessionId }),
      },
    );
    ...
    if (resp.data?.ok) {
      ...
      state.output.push({ type: 'error', text: resp.data?.error ?? 'Unknown error' });
    }
  } catch (err: any) {
    state.health = null;
    state.output.push({
      type: 'error',
      text: `Could not reach Claude Code server: ${err?.message ?? String(err)}`,
    });
  }
...
async function stopQuery(): Promise<void> {
...
// ---------------------------------------------------------------------------
// Styles (identical to panel.js — kept in sync)
// ---------------------------------------------------------------------------
...
const CSS = `
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .hint { color: var(--fg-tertiary, #555); font-size: 11px; margin-top: 6px; }
  .hint code { background: var(--surface-l2, #1e1e1e); padding: 1px 4px; border-radius: 3px; }
  ...
  .cmd-bar { display: flex; flex-wrap: wrap; gap: 6px; margin-bottom: 10px; }
  ...
  .session-row {
    ...
    padding: 5px 0; border-bottom: 1px solid var(--border-l1, #1e1e1e);
  ...
  .output { flex: 1; overflow-y: auto; padding: 8px 0; scroll-behavior: smooth; }
  ...
  .e-res { color: var(--fg-primary, #d4d4d4); white-space: pre-wrap; word-break: break-word; }
  .e-res code.il { background: rgba(167,139,250,0.15); color: #a78bfa; padding: 1px 5px; border-radius: 3px; font-size: 12px; }
  .e-res pre { background: #111; border: 1px solid #222; border-radius: 6px; padding: 10px 12px; margin: 6px 0; overflow-x: auto; white-space: pre; word-break: normal; }
  ...
  .e-info { color: var(--fg-secondary, #aaa); font-size: 12px; }
  .e-info code.il { background: rgba(167,139,250,0.15); color: #a78bfa; padding: 1px 4px; border-radius: 3px; font-size: 11px; }

  .spinner { display: flex; align-items: center; gap: 4px; margin-bottom: 10px; }
  .dot { color: #a78bfa; font-size: 10px; animation: blink 1.2s infinite; opacity: 0.3; }
  .dot:nth-child(2) { animation-delay: 0.4s; }
  .dot:nth-child(3) { animation-delay: 0.8s; }
  @keyframes blink { 0%, 100% { opacity: 0.2; } 50% { opacity: 1; } }
  .spin-text { font-size: 12px; color: var(--fg-tertiary, #555); margin-left: 6px; font-style: italic; }

  .input-row { display: flex; align-items: center; gap: 8px; border-top: 1px solid #1e1e1e; padding-top: 10px; flex-shrink: 0; }
  .ps-main { color: #a78bfa; font-size: 15px; flex-shrink: 0; line-height: 1; }

  .cmd-input {
    flex: 1; background: transparent; border: none; outline: none;
    color: var(--fg-primary, #e0e0e0); font-family: inherit; font-size: 13px;
  ...
  .btn-stop {
    ...
    color: #f87171; padding: 4px 10px; border-radius: 4px; cursor: pointer;
    font-family: inherit; font-size: 12px; flex-shrink: 0;
    -webkit-appearance: none; appearance: none;
  }
...
xai.on('chat.message', (msg: any) => {
  if (msg?.text?.includes('@claude-code')) {
    setTimeout(async () => {
      await checkHealth();
      render();
    }, 3000);
