...
| `device.audio` | device | Required for the `xai.audio.*` family (playback control, background audio, lock-screen / media-session controls, queue management). Host routes calls to `HTMLAudioElement` on web or the Capacitor AudioPlayer plugin on native. Also gates the `audio.*` sandbox-bridge action namespace. | Frontend host (`SandboxBridgeService` audio action router), `HTMLAudioElement` / Capacitor AudioPlayer | No OS prompt for playback itself. Background audio on iOS requires the host app's `UIBackgroundModes` entitlement — apps cannot grant themselves new entitlements at runtime. |
| `device.microphone` | device | Required for `navigator.mediaDevices.getUserMedia({ audio: true })` inside the sandbox iframe (voice recording, speech-to-text capture). | Frontend host (manifest-parser policy check at install), browser OS mic prompt on first use | Policy-level declaration enforced by the parser at install time. The iframe `sandbox="allow-scripts"` attribute does not by itself block `getUserMedia`, so host-side permission gating is what keeps an undeclared app from silently recording the user. |
